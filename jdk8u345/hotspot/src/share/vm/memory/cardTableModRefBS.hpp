...
protected:
  ...
  jbyte*          _th_byte_map;         // the h2 card marking array
#endif
...
protected:
  ...
  jbyte *byte_for(const void *p) const {
    ...
    if (_whole_heap.contains(p)) {
      ...
      jbyte *result = &th_byte_map_base[uintptr_t(p) >> th_card_shift];

      assert(result >= _th_byte_map &&
                 result < _th_byte_map + _th_byte_map_size,
  ...
  jbyte *byte_for(const void *p) const {
    assert(_whole_heap.contains(p),
           err_msg("Attempt to access p = " PTR_FORMAT " out of bounds of "
                   " card marking array's _whole_heap = [" PTR_FORMAT
                   "," PTR_FORMAT "]",
                   p2i(p), p2i(_whole_heap.start()), p2i(_whole_heap.end())));
    jbyte *result = &byte_map_base[uintptr_t(p) >> card_shift];
    assert(result >= _byte_map && result < _byte_map + _byte_map_size,
  ...
  jbyte *byte_after(const void *p) const { return byte_for(p) + 1; }
...
protected:
...
#ifdef TERA_CARDS
  CardArr *_th_lowest_non_clean;
  size_t *_th_lowest_non_clean_chunk_size;
  uintptr_t *_th_lowest_non_clean_base_chunk_index;
  volatile int *_th_last_LNC_resizing_collection;
#endif
  ...
  void get_LNC_array_for_space(Space *sp, jbyte **&lowest_non_clean,
  ...
  void process_stride(Space *sp, MemRegion used, jint stride, int n_strides,
  ...
  void process_chunk_boundaries(Space *sp, DirtyCardToOopClosure *dcto_cl,
                                MemRegion chunk_mr, MemRegion used,
                                jbyte **lowest_non_clean,
                                uintptr_t lowest_non_clean_base_chunk_index,
                                size_t lowest_non_clean_chunk_size);
...
public:
  ...
  inline void inline_write_region(MemRegion mr) { dirty_MemRegion(mr); }

protected:
  void write_region_work(MemRegion mr) { inline_write_region(mr); }

public:
  inline void inline_write_ref_array(MemRegion mr) { dirty_MemRegion(mr); }

protected:
  void write_ref_array_work(MemRegion mr) { inline_write_ref_array(mr); }

public:
  bool is_aligned(HeapWord *addr) { return is_card_aligned(addr); }

  // *** Card-table-barrier-specific things.

  template <class T>
  inline void inline_write_ref_field_pre(T *field, oop newVal) {}

  template <class T>
  inline void inline_write_ref_field(T *field, oop newVal, bool release) {
  ...
  enum PrecisionStyle { Precise, ObjHeadPreciseArray };
  ...
  const jbyte *byte_for_const(const void *p) const { return byte_for(p); }
  const jbyte *byte_after_const(const void *p) const { return byte_after(p); }
    ...
    if (p >= _byte_map && p < _byte_map + _byte_map_size) {
      ...
      assert(_whole_heap.contains(result),
             err_msg("Returning result = " PTR_FORMAT " out of bounds of "
                     " card marking array's _whole_heap = [" PTR_FORMAT
                     "," PTR_FORMAT ")",
                     p2i(result), p2i(_whole_heap.start()),
                     p2i(_whole_heap.end())));
      return result;
    } else {
      size_t delta = pointer_delta(p, th_byte_map_base, sizeof(jbyte));

      HeapWord *result = (HeapWord *)(delta << th_card_shift);
      assert(_th_whole_heap.contains(result),
             err_msg("Returning result = " PTR_FORMAT
                     " out of bounds of card marking arrays "
                     "_whole_heap = [" PTR_FORMAT "," PTR_FORMAT "]",
                     p2i(result), p2i(_th_whole_heap.start()),
                     p2i(_th_whole_heap.end())));
...
public:
  CardTableModRefBSForCTRS(MemRegion whole_heap, int max_covered_regions)

...
#ifdef TERA_MINOR_GC
template <class T> inline bool PSScavenge::h2_should_scavenge(T* p) {
	T heap_oop = oopDesc::load_heap_oop(p);
...
template <class T> inline bool PSScavenge::h2_should_trace(T* p) {
	T heap_oop = oopDesc::load_heap_oop(p);
...
inline void PSScavenge::copy_and_push_safe_barrier(PSPromotionManager* pm,
  ...
  assert(Universe::heap()->is_in_reserved(o) ||  Universe::teraHeap()->is_obj_in_h2(o), 
			err_msg("Object should be in object space or in TeraCache. O = %p", (HeapWord *)o));

  // If `o` has been copied to the new address, then forwardee of o is
  // not empty, and is_forwarded is true, At this point, just take out
  // the new address directly. If there is no forward, then do a
  // forward. 
#ifdef TERA_MINOR_GC
  ...
  // The new address obtained, whether it is copied this time or
  // generated by the previous copy forwarding, update to the
  // reference. 
  ...
  if ((!PSScavenge::is_obj_in_young((HeapWord*)p)) &&
    ...
    assert(!Universe::teraHeap()->is_field_in_h2((void*) p), "Sanity");
#endif // TERA_MINOR_GC
...
#ifdef TERA_MINOR_GC
    DEBUG_ONLY( if (EnableTeraHeap) { assert(!Universe::teraHeap()->is_field_in_h2((void *)p), "Error"); })
#endif // TERA_MINOR_GC
...
#ifdef TERA_MINOR_GC
    DEBUG_ONLY(if (EnableTeraHeap) { assert(!Universe::teraHeap()->is_field_in_h2(p), "Not from meta-data?");});
#endif // TERA_MINOR_GC
    if (PSScavenge::should_scavenge(p)) {
...
#ifdef TERA_MINOR_GC
      if (EnableTeraHeap && Universe::teraHeap()->is_obj_in_h2(o)) return;
#endif // TERA_MINOR_GC

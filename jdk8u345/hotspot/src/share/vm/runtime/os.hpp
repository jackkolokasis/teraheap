...
class os: AllStatic {
 ...
 public:
  ...
  // Helper function to create a new file with template
  // jvmheap.XXXXXX. 
  // Returns a valid fd on success or else returns -1
  static int create_file_for_heap(const char* dir);
  // Map memory to the file referred by fd. This function is slightly
  // different from map_memory() and is added to be used for
  ...
  static char* replace_existing_mapping_with_file_mapping(char* base,
                                                          size_t size,
                                                          int fd);

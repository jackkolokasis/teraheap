...
private[memory] class StorageMemoryPool(
   ...
   * `spaceToFree` bytes.  Note: this method doesn't actually reduce
   * the pool size but relies on the caller to do so.
   *
   * @return number of bytes to be removed from the pool's capacity.
   */
  def freeSpaceToShrinkPool(spaceToFree: Long): Long = lock.synchronized {
    val spaceFreedByReleasingUnusedMemory = math.min(spaceToFree, memoryFree)
    // JK: Storage memory area needs to release the memory of
    // remainingSpaceToFree size.
    val remainingSpaceToFree = spaceToFree - spaceFreedByReleasingUnusedMemory

    // JK: Greater than 0 means that there is no available memory in
    // the current Storage memory area. You need to clear the block in
    // the Storage memory area to implement Shrink operation.

...
// Signed variants of alignment helpers. There are two versions of
// each, a macro for use in places like enum definitions that require
// compile-time constant expressions and a function for all other
// places so as to get type checking.

// Using '(what) & ~align_mask(alignment)' to align 'what' down is
// broken when 'alignment' is an unsigned int and 'what' is a wider
// type. The & operation will widen the invered mask, and not sign
// extend it., leading to a mask with zeros in the most significant
// bits/ The use of align_mask_widened() solve this problem
...
#define align_down_(size, alignment) \
    ((size) & ~align_mask_widened((alignment), (size)))

#define align_up_(size, alignment) \
    (align_down_((size) + align_mask(alignment), (alignment)))

#define is_aligned_(size, alignment) \
    (((size) & align_mask(alignment)) == 0)
...
// Align down with a lower bound. If the aligning results in 0, return 
// 'alignment'.
template <typename T, typename A>
...
// 2. If addr is above the page_address the start of the *next* will
//    be returned
// 3. Otherwise, if addr is below the page_address the start of the
...
inline T* clamp_address_in_page(T* addr, T* page_address, size_t page_size) {
    if (align_down(addr, page_size) == align_down(page_address, page_size)) {
        ...
        // address is above specified page, return start of the next
        // page

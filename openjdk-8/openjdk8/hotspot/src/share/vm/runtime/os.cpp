...
static const char* errno_to_string (int e, bool short_text) {
  ...
  #define DEFINE_ENTRY(e, text) { e, #e, text },
    ...
    static const struct {
        ...
        // The following enums are not define to all platforms
#ifdef ESTALE
...
//JK: Add file_desc as extra argument to support -XX:AllocateHeapAt
//operation
char* os::reserve_memory(size_t bytes, 
  ...
  if (file_desc != -1) {
      // Could have called pd_reserve_memory() followed by
      // replace_existing_mapping_with_file_mapping(), but AIX may use
      // SHM in which case its more trouble to detach the segment and
      // remap memory to the file
...
// JK: Modified this function to support heap allocation on the file
// refered by file_desc
...
// JK: Implementation of pretouch memory function

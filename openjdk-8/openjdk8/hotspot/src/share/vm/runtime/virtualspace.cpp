...
// JK: Need to fix the "initialize" function
ReservedSpace::ReservedSpace(size_t size, size_t alignment,
                             bool large,
                             char* requested_address,
                             const size_t noaccess_prefix): _fd_for_heap(-1) {
  initialize(size+noaccess_prefix, alignment, large, requested_address, false);
}
...
ReservedSpace::ReservedSpace(char* base, size_t size, size_t
        alignment, bool special, bool executable): _fd_for_heap(-1) {
...
// Helper method.
// JK: Helper method
static bool unmap_or_release_memory(char* base, size_t size, bool is_file_mapped) {
    if (is_file_mapped) {
  ...
  if (base != NULL) {
    ...
    if (special) {
      if (!os::release_memory_special(base, size)) {
        ...
        // JK: Replace the commented code with this
...
void ReservedSpace::initialize(size_t size,
    ...
    if (special && _fd_for_heap != -1) {
        ...
        if (UseLargePages && !(FLAG_IS_DEFAULT(UseLargePages) ||
            ...
            std::cerr << "Ignoring UseLargePages since large page support is up to the file system of the backing file for Java heap" << std::endl;
        }
    }
    ...
    if (base == NULL) {
        ...
        if ((((size_t) base) & (alignment -1)) != 0) {
            // base not aligned retry
            unmap_or_release_memory(base, size, _fd_for_heap != -1 /*is file mapped */);

            // Make sure that size is alligned
    ...
    _size = size;
    _alignment = alignment;
    // JK: remove noaccess_prefix
    //_noaccess_prefix = noaccess_prefix;
    // if heap is reserved with a backing file, the entire space has
    // been commited. So set the special flag to true
    if (_fd_for_heap != -1) {
        _special = true;
    }
  }
...
void ReservedHeapSpace::establish_noaccess_prefix() {
    assert(_alignment >= (size_t)os::vm_page_size(), "must be at least page size");
    ...
    if (base() && base() + _size > (char *)OopEncodingHeapMax) {
        if (true
            ...
            std::cerr << "Protected page at the reserved heap base" << std::endl;
...
// Tries to allocate memory of size 'size' at address
// requested_address with alignment 'alignment'. Does not check
// whether the reserved memory actually is at requested_address, as
// the memory returned might still fullfill the wishes of the caller.
// Assume that the memory is aligned to 'alignment'.
// NOTE: If ReservedHeapSpace is already points to some reserved space
// this is freed, first.
void ReservedHeapSpace::try_reserve_heap(size_t size,
                                       ...
                                       char* requested_address) {
    if (_base != NULL) {
        // we tried before and we did not like the address delivered.
        release();
    }
    ...
    if (special && _fd_for_heap != -1) {
      ...
      if (UseLargePages && (!FLAG_IS_DEFAULT(UseLargePages) ||
        ...
        // log_debug(gc, heap)("Cannot allocate large pages for Java Heap when AllocateHeapAt option is set.");                                          
          std::cerr << "Cannot allocate large pages for Java Heap when AllocateHeapAt option is set." << std::endl;
      }
    }
    char* base = NULL;
    ...
    std::cerr << "Trying to allocate at address " << std::endl;
    ...
    if (base == NULL) {
        // Failed; try to reserve regular memory
        if (UseLargePages && (!FLAG_IS_DEFAULT(UseLargePages) ||
                              !FLAG_IS_DEFAULT(LargePageSizeInBytes))) {
            // log_debug(gc, heap, coops)("Reserve regular memory without large pages");
            std::cerr << "Reserve regular memory without large pages" << std::endl;
        }
    ...
    // If heap is reserved with a backing file, the entire space has
    // been commited. So set the _special flag to true
    if (_fd_for_heap != -1) {
        _special = true;
    }
...
void ReservedHeapSpace::try_reserve_range(char *highest_start,
    ...
    // Try to attach point from top to bottom
...
// Helper function for heap allocation. Return an array with addresses
...
static char** get_attach_addresses_for_disjoint_mode() {
    ...
    // Sort out addresses smaller than HeapBaseMinAddress. This
...
void ReservedHeapSpace::initialize_compressed_heap(const size_t size,
                                                    size_t alignment,
                                                    bool large) {
    ...
    // Keep Heap at BaseMinAddress
    if (_base == NULL) {
        // Try to allocate heap at addresses that allow efficient oop
        // compression.
        // Different shemes are tried, in order of decreasing
        // optimization pottential.
        //
        // For this, try_reserve_heap() is called with the desire base
        // heap addresses.
        // A call to the os layer to allocate at given address can
        // return memory at a different address than requested. Still
        // this might be memory at a useful address.
        // try_reserve_heap() always returns this allocated memory, as
        // only here the criteria for a good heap are checked.
        ...
        while (addresses[i] && ((_base == NULL) || 
                    (_base + size >  (char *)OopEncodingHeapMax)))  {  // Not disjoint address.
        ...
        if (_base == NULL) {
            ...
            std::cerr << "Trying to allocate at address NULL heap of size "  << std::endl;
            // TODO : JK Check again if this is correct
            initialize(size + noaccess_prefix, alignment, large, NULL, false);
        }
    }
}

// JK: Continue from here
ReservedHeapSpace::ReservedHeapSpace(size_t size,
                                     size_t alignment,
                                     bool large,
                                     //char* requested_address
                                     const char* heap_allocation_directory) : ReservedSpace() {
...
//                  lcm(os::vm_page_size(), alignment) : 0) {
    ...
    if (UseCompressedOops) {
        ...
        // initialize(size, alignment, large, NULL, 0, false);
...
// Reserve space for code segment. Same as Java heap only we mark this
// as executable.
...
bool VirtualSpace::expand_by(size_t bytes, bool pre_touch) {
  ...
  if (lower_needs > 0) {
    ...
    // JK:
    if (!commit_expanded(lower_high(), lower_needs, 
                         lower_alignment(), pre_touch, _executable)) {
        return false; 
    }
  ...
  if (middle_needs > 0) {
    ...
    if (!commit_expanded(middle_high(), middle_needs,
                         middle_alignment(), pre_touch, _executable)) {
        return false;
    }
    _middle_high += middle_needs;
  }
  ...
  if (upper_needs > 0) {
    ...
    // }
    if (!commit_expanded(upper_high(), upper_needs, 
                         upper_alignment(), pre_touch, _executable)) {
        return false;
    }
    _upper_high += upper_needs;
  }

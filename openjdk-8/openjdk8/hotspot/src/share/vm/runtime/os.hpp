...
class os: AllStatic {
 ...
 private:
  ...
  // JK: Same as pd_attempt reserve_memory_at(size_t bytes, char*
  // addr) but support file heap referred by file_desc
 ...
 public:
  ...
  // JK: Add file_desc as extra argument in reserve_memory function
  static char*  reserve_memory(size_t bytes, char* addr = 0,
                               size_t alignment_hint = 0,
                               int file_desc = -1);
  static char*  reserve_memory(size_t bytes, char* addr,
                               size_t alignment_hint, MEMFLAGS flags);
  // JK: Add file_desc as extra argument in reserve_memory_aligned
  static char*  reserve_memory_aligned(size_t size, 
                                       size_t alignment, 
                                       int file_desc = -1);
  ...
  // (exclusive) to make the OS back the memory range with actual
  // memory. Current implementation may not touch the last page if
  // unaligned addresses are passed
  ...
  // JK: Helper function to create a new file with template
  // jvmheap.XXXXXX.
  // Returns a valid fd on success or else returns -1
  static int create_file_for_heap(const char* dir);

  // JK: Map memory to the file registered by fd. This function is
  // slightly different from map_memory() and is added to be used for
  // implementation of -XX:AllocateHeapAt
  static char*  map_memory_to_file(char* base, size_t size, int fd);

  // JK: Replace existing memory mapping with file mapping
  ...
  // JK: Replacement for strerror().
  // Will return the english description of the error (e.g., "File not
  // found", as suggested in the POSIX standar. Will return "Unknown
  // error" for an unknown errno value. Will not attempt to localize
  // the returned string. Will always return a valid string which is a
  // static constant. Will not change the value of errno.
  static const char* strerror(int e);

  // JK: Will return the literalized version of the given errno (e.g
  // "EINVAL" for EINVAL).

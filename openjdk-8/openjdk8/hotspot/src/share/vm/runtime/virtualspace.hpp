...
class ReservedSpace VALUE_OBJ_CLASS_SPEC {
 ...
 // JK: Change from private to protected
  protected:
   char*  _base;
   size_t _size;
 ...
 public:
  // Constructor
  ReservedSpace();
  // Initialize the reserve space with the ginven size. If
  // preferred_page_size is set, use this as minimum page
  // size/alignment. This may waste some space if the given space is
  // not aligned to that value, as the reservation will be aligned up
  // to the final alignment in this case.
...
class ReservedHeapSpace : public ReservedSpace {
    ...
    public:
        // Constructor. Tries to find a heap that is good for
        // compressed oops.
        // heap_allocation_directory is the path to the backing memory
        // for Java Heap. When set, Java heap will be allocated on the
        // device which is managed by the filesystem where the
        // directory resides.
        //ReservedHeapSpace(size_t size, 
        //                  size_t forced_base_alignment, 
        //                  bool large, 
        //                  char* requested_address);
        ...
        // Returns the base to be used for compression, i.e. so that
        // null can be encoded safely and implicit null checks can
        // work

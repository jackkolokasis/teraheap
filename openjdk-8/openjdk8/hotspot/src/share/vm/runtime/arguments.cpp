...
const char*  Arguments::_tc_log_filename        = NULL;
...
bool Arguments::check_vm_args_consistency() {
  ...
  // JK: Check for ALlocate Heap flag
  if (!FLAG_IS_DEFAULT(AllocateHeapAt)) {
    if ((UseNUMAInterleaving && !FLAG_IS_DEFAULT(UseNUMAInterleaving)) || (UseNUMA && !FLAG_IS_DEFAULT(UseNUMA))) { 
        ...
        std::cout << "NUMA support for Heap depends on the file system when AllocateHeapAt option is used.\n";
    }
    // status = false
  }
...
jint Arguments::parse_each_vm_init_arg(const JavaVMInitArgs* args,
  ...
  for (int index = 0; index < args->nOptions; index++) {
	...
	} else if (match_option(option, "-Xlogtc:", &tail)) {
		// Redirect TeraCache output to the file. -Xlogtc:<filename>
		// ostream_init_log(), when called will use this filename to initialize
		// a fileStream)
		_tc_log_filename = strdup(tail);
    // JNI hooks

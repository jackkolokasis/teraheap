...
void HeterogeneousGenerationSizer::initialize_flags() {
    ...
    if (MaxNewSize > reasonable_max) {
        if (FLAG_IS_CMDLINE(MaxNewSize)) {
            std::cout << "Setting MaxNewSize to " << (size_t)reasonable_max
                      << " based on dram available (calculation = align("
                      << calc_str.buffer() << "))" << std::endl;
            ...
            std::cout << "Setting MaxNewSize to " << (size_t)reasonable_max
                      << " based on dram available (calculation = align("
                      << calc_str.buffer() << "))" 
                      << "Dram usage can be lowered by setting MaxNewSize to a lower value "
                      << std::endl;
    ...
    if (NewSize > reasonable_max) {
        if (FLAG_IS_CMDLINE(NewSize)) {
            std::cout << "Setting NewSize to " << (size_t)reasonable_max
                      << " based on dram available (calculation = align("
                      << calc_str.buffer() << "))" << std::endl;

...
PSFileBackedVirtualSpace::PSFileBackedVirtualSpace(ReservedSpace rs, size_t alignment, const char* path):
    PSVirtualSpace(rs, alignment), _file_path(path), _fd(-1), _mapping_succeeded(false) {
...
bool PSFileBackedVirtualSpace::initialize() {
    ...
    // _mapping_succeedded is false if we return before this point
    // expand calls later and check value of this flag and return error if it is false
...
bool PSFileBackedVirtualSpace::expand_by(size_t bytes) {
    assert(special(), "Since entire space is commited at initialization, _special should always be true for PSFileBackedVirtualSpace");

    // If mapping did not succeed during initialization return false
...
size_t PSFileBackedVirtualSpace::expand_intro(PSVirtualSpace* space, size_t bytes) {
    // not supported. Since doing this will change page mapping which will lead
    // to large TLB penalties.
    return 0;
}

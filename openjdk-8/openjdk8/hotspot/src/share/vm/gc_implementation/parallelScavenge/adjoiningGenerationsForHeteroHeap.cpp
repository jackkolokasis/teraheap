 ...
 * DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER.(AdjoiningVirtualSpaces)
 *
...
// Create two virtual spaces (HeteroVirtualSpaces), low() on storage
// device, high() on dram.
...
AdjoiningGenerationsForHeteroHeap::AdjoiningGenerationsForHeteroHeap(ReservedSpace old_young_rs,
    ...
    _total_size_limit(policy->max_heap_byte_size()) {
        ...
        size_t init_old_byte_size   = policy->initial_gen1_size();
        size_t min_old_byte_size    = policy->min_gen1_size();
        size_t max_old_byte_size    = policy->max_gen1_size();
        size_t init_young_byte_size = policy->initial_gen0_size();
        size_t min_young_byte_size  = policy->min_gen0_size();
        size_t max_young_byte_size  = policy->max_gen0_size();

        // Create HeteroVirtualSpace which is composed of
        // non-overlapping virtual space
        HeteroVirtualSpaces* hetero_virtual_spaces = 
            new HeteroVirtualSpaces(old_young_rs, min_old_byte_size,
        ...
        assert(UseAdaptiveGCBoundary, "Should be used when UseAdaptiveGCBoundary is true");

        // Initialize the virtual spaces. Then pass a virtual space to each
        // generation for initialization of the generation.
        // Does the actuall ctreation of the virtual spaces
        hetero_virtual_spaces->initialize(max_old_byte_size, init_old_byte_size,
                                          init_young_byte_size);

        _young_gen = new ASPSYoungGen(hetero_virtual_spaces->high(),
                                     hetero_virtual_spaces->high()->committed_size() /* initial_size */,
                                     min_young_byte_size,
                                     hetero_virtual_spaces->max_young_size());
        
        _old_gen = new ASPSOldGen(hetero_virtual_spaces->low(),
                                  hetero_virtual_spaces->low()->committed_size() /* initial_size */,
                                  min_old_byte_size,
...
size_t AdjoiningGenerationsForHeteroHeap::required_reserved_memory(GenerationSizer *policy) {
    // This is the size that young gen can grow to, when AdaptiveGCBoundary is
    // true.
    size_t max_yg_size = policy->max_heap_byte_size() - policy->min_gen1_size();
    // This is the size that old gen can grow to, when AdaptiveGCBoundary is
    // true.
    size_t max_old_size = policy->max_heap_byte_size() - policy->min_gen0_size();

    return max_yg_size + max_old_size;
}

// We override this function size of reservedspace here is more than heap size
...
void AdjoiningGenerationsForHeteroHeap::HeteroVirtualSpaces::initialize(size_t initial_old_reserved_size,
    ...
    // Initially we only assign 'initial_old_reserved_size' of the reserved
    // space to old virtual space
...
// Since the virtual spaces are non-overlaping, there is no boundary as such.
// We replicate the same behavior and maintain the same invariants as base class
// "AdjoiningVirtualSpaces" by increasing old generation size and decreasing
// young generation size by same amount
bool AdjoiningGenerationsForHeteroHeap::HeteroVirtualSpaces::adjust_boundary_up(size_t change_in_bytes) {
    ...
    // Check if the old generation expanded
    bool old_expanded = false;

    // 1. Try to expand old within its reserved space.
    // If old generation has enough space
    if (uncommitted_in_old != 0) {
        // Simple expand within old space
        if (!old_vs()->expand_by(uncommitted_in_old)) {
            // return false if the expansion fail
            return false;               
        }

        old_expanded = true;
        bytes_needed -= uncommitted_in_old;
        // Check if the amount of needed bytes filled by the uncommitted  old
        // generation size
        if (bytes_needed == 0) {
            return true;
        }
    ...
    // 2. Get ucommited memory from Young Virtualspace
...
bool AdjoiningGenerationsForHeteroHeap::HeteroVirtualSpaces::adjust_boundary_down(size_t change_in_bytes) {
    ...
    //1. Try to expand new within its reserved space.

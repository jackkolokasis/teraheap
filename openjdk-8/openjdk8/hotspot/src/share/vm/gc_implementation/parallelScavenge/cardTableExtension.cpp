 ...
 public:
  ...
  virtual void do_object(oop obj) {
    ...
    if (object_check.has_unmarked_oop()) {
      assertf(_card_table->addr_is_marked_imprecise(obj), "Found unmarked young_gen object");
    }
  }
};
...
class CheckForPreciseMarks : public OopClosure {
 ...
 protected:
  template <class T> void do_oop_work(T* p) {
    ...
    if (_young_gen->is_in_reserved(obj)) {
      assertf(_card_table->addr_is_marked_precise(p), "Found unmarked precise oop");
		...
		while (current_card < worker_end_card) {
			...
			while (current_card < worker_end_card && !tc_card_is_clean(*current_card, is_scavenge_done)) {
				...
				if (current_card < worker_end_card) {
                    ...
                    if (!Universe::teraCache()->check_if_valid_object(addr_for(current_card)-1)) {
                        end_of_last_object = Universe::teraCache()->get_last_object_end(addr_for(current_card)- 1);
                    }
                    else {
                        last_object_in_dirty_region = start_array->tc_object_start(addr_for(current_card)-1);

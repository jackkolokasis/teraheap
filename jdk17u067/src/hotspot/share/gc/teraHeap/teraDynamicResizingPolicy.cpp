...
#define WINDOW_INTERVAL ((10LL * 1000))
#define MIN_H1_SIZE ((27LL * 1024 * 1024 * 1024 * 0.3))
...
void TeraDynamicResizingPolicy::gc_end(double gc_duration) {
  unsigned long long gc_iowait_end = 0, gc_cpu_end = 0;
...
uint64_t TeraDynamicResizingPolicy::get_device_active_time(const char* device) {
  ...
  char line[1024];
  while (fgets(line, sizeof(line), stat_file) != nullptr) {
...
TeraDynamicResizingPolicy::state TeraDynamicResizingPolicy::action() {
  ...
  unsigned long long iowait_end, cpu_end;
  ...
  //fprintf(stderr, "h1_used_at_last_gc(GB) = %lf\n", (double) Universe::heap()->used_at_last_gc()/1024/1024/1024.0);
  //fprintf(stderr, "h1_free_at_last_gc(GB) = %lf\n", (double) Universe::heap()->free_at_last_gc()/1024/1024/1024.0);
  
  double free_at_last_gc = (double) Universe::heap()->free_at_last_gc() / Universe::heap()->capacity();

  if (free_at_last_gc < FREE_THRESHOLD) {
  ...
  if (iowait_time_ms > gc_time && device_active_time_ms > 0) {
    ...
    if (prev_action == S_GROW_H1) {
      prev_action = S_MOVE_BACK;
      return S_MOVE_BACK;
    }

    if ((Universe::heap()->capacity() > MIN_H1_SIZE)) {

      prev_action = S_SHRINK_H1;
      return S_SHRINK_H1;
    }

    if (Universe::heap()->capacity() <= MIN_H1_SIZE && prev_action == S_SHRINK_H1) {

 ...
 *
 * Implementation of Persistent Malloc Library
 *
 ***************************************************
 */
...
/**
 * Hidden bytes added to each allocation. 
 * The metadata we keep for each allocation is 64 bytes in size so
 * when we return the address after the hidden bytes, it is also
 * 64-byte aligned.
 */
struct clump {
...
pmemalloc_coalesce_free(void *pmp)
{
    ...
    while (clp->size) {
        ...
        if (state == PMEM_STATE_FREE) {
            ...
            pmem_persist(firstfree, sizeof(*firstfree));
            firstfree = lastfree = NULL;
            csize = 0;
        } else {
            firstfree = lastfree = NULL;
...
/*
 * @desc Setup a Persistent Memory pool for use
 *       This function must be called before any other pmem functions.
 *
 * @param path  Path to the file which will contain the memory pool If the file
 *              doesn't exist, it is created.  If it exists, the state of the 
 *              memory pool is initialized from the file.
 * @param size  Size of the memory pool in bytes
 *              The size is only used when creating the memory pool the first
 *              time (the file created will be extended to that size).  The
 *              smallest size allowed is 1 meg.  The largest size allowed is
 *              whatever the underlaying file system allows as a max file size.
 *
 * @ret         An opaque memory pool handle is returned on success.  That 
...
{
    void *pmp;                             /* Persistent Memory pointer                 */
    int fd=-1;                             /* File descriptor                           */
    int err;                               /* Error value                               */ 
    struct stat stbuf;                     /* Status structure                          */
	int is_pmem;						   /* 1 for persistent memory, 0 otherwise	    */
	size_t mapped_len;					   /* Mapped file size 							*/
     ...
     * map the file
     */
  	pmp = pmem_map_file(path, size, PMEM_FILE_CREATE, 0666, &mapped_len,
                           &is_pmem);
    ...
    while (clp->size) {
        ...
        if (state == PMEM_STATE_FREE && nsize <= sz) {
            ...
            if (leftover >= PMEM_CHUNK_SIZE * 2) {
                ...
                pmem_persist(newclp, sizeof(*newclp));
                for (i = 0; i < PMEM_NUM_ON; i++) {
                    clp->on[i].off = 0;
                    clp->on[i].ptr_ = 0;
                }
...
pmemalloc_activate(void *pmp, void *ptr_)
{
     ...
     * 6. persist *clp
     */
    pmem_persist(PMEM(pmp, ptr_), clp->size - PMEM_CHUNK_SIZE);
